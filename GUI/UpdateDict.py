...
class WaitingWindow(tk.Toplevel):
    def __init__(self, top=None):
        ...
        self.FrameDict.place(relx=0.015, rely=0.029, relheight=0.131
                , relwidth=0.975)
        ...
        self.FrameIdx.place(relx=0.017, rely=0.32, relheight=0.131, relwidth=0.975)
        ...
        self.LabelIdxStatus.place(relx=0.632, rely=0.222, height=25
                , width=118)
        self.LabelIdxStatus.configure(text='''Pending...''')

        self.FrameSample = tk.Frame(self)
        self.FrameSample.place(relx=0.017, rely=0.465, relheight=0.131
                , relwidth=0.975)
        ...
        self.LabelSampleStatus.place(relx=0.632, rely=0.222, height=25
                , width=118)
...
class TopUpdateDict(tk.Toplevel):
    def __init__(self, top=None, pathDir=None):
        ...
        self.FrameStatus.place(relx=0.017, rely=0.003, relheight=0.099
                , relwidth=0.975)
        ...
        self.LabelStatus.configure(text='''Please choose the following directories and file''')

        self.FrameDict = tk.Frame(self)
        self.FrameDict.place(relx=0.033, rely=0.086, relheight=0.28
                , relwidth=0.942)
        ...
        self.TextDict.place(relx=0.257, rely=0.307, relheight=0.362
                , relwidth=0.772)
        ...
        self.FrameVCF.place(relx=0.033, rely=0.326, relheight=0.28
                , relwidth=0.942)
        ...
        self.TextVCF.place(relx=0.257, rely=0.307, relheight=0.424
                , relwidth=0.772)
        ...
        self.FrameSample = tk.Frame(self)
        self.FrameSample.place(relx=0.033, rely=0.566, relheight=0.28
                , relwidth=0.942)
        ...
        self.TextSample.place(relx=0.257, rely=0.307, relheight=0.424
                , relwidth=0.772)
    ...
    def pickDictFile(self):
        self.oldDicts = filedialog.askdirectory()#filedialog.askopenfilename(filetypes=(("json files","*.json"),("all files","*.*")))
    ...
    def createTempPAM(self, pam, pamFile): #TODO to prevent pam with no guide len in the name, put 25 Ns , check if positions are correct
        parts = pamFile.split("-")
        if parts[0] != pam:
            with open (self.pathDir+"/pam/tempPAM.txt", "w") as tempPAM:
                fullPAM = "".join(["N"]*int(parts[0][0:2])) + pam + " " + str(len(pam))
                tempPAM.write(fullPAM)
        else:
            with open (self.pathDir+"/pam/tempPAM.txt", "w") as tempPAM:
                fullPAM = pam + "".join(["N"]*int(parts[1][0:2])) + " -" + str(len(pam))
                tempPAM.write(fullPAM)
    ...
    def confirm(self):
        ...
        for i, VCFFile in enumerate(os.listdir(self.VCFDir)):
            with gzip.open(self.VCFDir+"/"+VCFFile, 'rb') as targets:
                ...
                line = targets.readline()#Save CHROM [0], POS[1], REF [3], ALT [4], List of Samples [9:]
            ...
            if len(dict_vcf) != i+1:
                ...
                messagebox.showerror("WARNING!","No dictionaries associated to "+VCFFile)
                ww.destroy()
                return
        # print(dict_vcf)
        ...
        for oldDict, VCFFile in dict_vcf:
            ...
            updateDictionary(self.oldDicts+"/"+oldDict, self.VCFDir+"/"+VCFFile)
        ...
        for item in os.listdir(self.pathDir+"/Genomes/"+genomeEnr+"/"):
            chrom = item.split('.')[0]
            os.rename(self.pathDir+"/Genomes/"+genomeEnr+"/"+item, self.pathDir+"/Genomes/"+genomeEnr+"/"+chrom+".fa")
        os.system("crispritz.py add-variants "+self.VCFDir+"/ "+self.pathDir+"/Genomes/"+genomeEnr+"/")
        ...
        move(self.pathDir+"/variants_genome/SNPs_genome/" + genomeEnr + '_enriched', self.pathDir+"/Genomes/"+genomeEnr)
