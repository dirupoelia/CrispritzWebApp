...
def findVars(s):
    ...
    if length > 1: #if there are more than one variant for a certain chr position
        for i in range(0,length-2,2):
            result[s[posVar[i]+1:posVar[i+1]]] = posVar[i] #get a single variation at a time
...
def getSamples(entry, variations, target):
    ...
    if len(keys) > 1: #if there are more than one variant for a certain chr position
        if target == keys[0]:
            return entry[:variations[target]], 0, variations[keys[0]] #get first variation and its position
        elif target == keys[len(keys)-1]:
            return entry[variations[keys[len(keys)-2]]+5:variations[target]], variations[keys[len(keys)-2]]+5, variations[target] #get last variation and its position
        else:
            return entry[variations[keys[keys.index(target)-1]]+5:variations[target]], variations[keys[keys.index(target)-1]]+5, variations[target] #get in between variation and its position
    else:
        return entry[:variations[keys[0]]], 0, variations[keys[0]] #get the only variation and its position
