...
with open (sys.argv[1]) as t, open(name_output + '.minmaxdisr.txt','w+') as result:
    result.write('#Bulge type\tcrRNA\tDNA\tChromosome\tPosition\tDirection\tMismatches\tBulge Size\tTotal\tMin_mismatches\tMax_mismatches\tPam_disr\tPAM_gen\tVar_uniq\n')
    for line in t:
        found_iupac = False
        found_iupac_pam = False
        line = line.strip().split('\t')
        max_mm = int(line[6])
        sum_mms_bul = int (line[6]) + int(line[7])
        ...
        for pos, char in enumerate(line[2][pam_begin:pam_end]):
            if char in iupac_code:      
                ...
                if diff_res:
                    pam_disr = pam_disr + ','.join(diff_res) + '; '
                else:
                    pam_disr = pam_disr + '-; '
