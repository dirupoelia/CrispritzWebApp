...
if [ ${19} = 'ref' ]; then
    ...
    python3 ${20}PostProcess/cluster.dict.py $jobid.targets.txt 'addGuide' 'True' 'False' guides.txt # > $jobid.targets.cluster.txt
    echo 'End cluster ref'

    echo 'Start extraction top1 ref'
    python3 ${20}PostProcess/extract_top.py $jobid.targets.cluster.txt $jobid  # > $jobid.top_1.txt
    echo 'End extraction top1 ref'

    echo 'Start Scoring'
    echo 'Calculating Scores... Step [2/3]' >>  output.txt
    python3 ${20}PostProcess/scores_guide_table.py $jobid.top_1.txt $3 pam.txt guides.txt
    echo 'End Scoring'

    echo 'Start summary by pos-guide'
    echo 'Creating Summaries... Step [3/3]' >>  output.txt
    type_post='No'      
    python3 ${20}PostProcess/summary_by_guide_position.py $jobid.targets.cluster.txt $7 $8 $9 guides.txt $jobid $type_post
    ...
    python3 ${20}PostProcess/cluster.dict.py $jobid.targets.minmaxdisr.txt 'no' 'True' 'False' guides.txt 'total' 'orderChr' # > $jobid.targets.minmaxdisr.cluster.txt
    ...
    python3 ${20}PostProcess/summary_by_guide_position.py $jobid.total.cluster.txt $7 $8 $9 guides.txt $jobid 'Uniq'  
    echo 'End summary by guide and position' 


    #Summary guide
    echo 'Start summary by guide'  
    python3 ${20}PostProcess/summary_by_guide.py $jobid.samples.annotation.txt $7 $8 $9 guides.txt $jobid 'no'
    ...
    if [ ${13} = 'True' ]; then
        ...
        while IFS= read -r line || [ -n "$line" ]; do    
            ...
            printf %s\\n $(seq 0 $total) | xargs -n 1 -P $proc -I % python3 ${20}PostProcess/populations_distribution.py $jobid.PopulationDistribution.txt % $line
            ...
            #Clusterize results
            python3 ${20}PostProcess/cluster.dict.py $jobid.targets.$line.txt 'no' 'True' 'True' guides.txt 'total' 'addForFinal'    #> $jobid.targets.$line.cluster.txt
    ...
    python3 ${20}PostProcess/cluster.dict.py $jobid.total.txt 'no' 'True' 'True' guides.txt 'total' 'orderChr'  #-> 03/03 il cluster ottenuto è ordinato per chr, uso questo per annotation
    ...
    #Summary guide
    echo 'Start summary by guide'  
    python3 ${20}PostProcess/summary_by_guide.py $jobid.samples.annotation.txt $7 $8 $9 guides.txt $jobid 'Uniq'
    echo 'End summary by guide'
