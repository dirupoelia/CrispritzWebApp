...
def createTempPAM(fileDir, pam, pamFile): #TODO to prevent pam with no guide len in the name, put 25 Ns , check if positions are correct
    parts = pamFile.split("-")
    if parts[0] != pam:
...
for item in os.listdir(fileDir+"/Genomes/"+genomeEnr+"/"):
    chrom = item.split('.')[0]
    print(chrom)
    os.rename(fileDir+"/Genomes/"+genomeEnr+"/"+item, fileDir+"/Genomes/"+genomeEnr+"/"+chrom+".fa")
print(os.listdir(fileDir+"/Genomes/"+genomeEnr+"/"))
os.system("crispritz.py add-variants "+VCFDir+"/ "+fileDir+"/Genomes/"+genomeEnr+"/")
...
for item in os.listdir(fileDir+"/variants_genome/SNPs_genome/" + genomeEnr + '_enriched/'):
    copy(fileDir+"/variants_genome/SNPs_genome/" + genomeEnr + '_enriched/'+item, fileDir+"/Genomes/"+genomeEnr+"/"+item)
